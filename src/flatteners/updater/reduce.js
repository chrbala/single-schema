...
const MISSING_KEY_ERROR = key => `Attempted to access nonexistant key ${key}`;
  ...
  () =>
    ({ subscribe: scopedSubscribe, getState: scopedGetstate }: ScopeType) => {
      ...
      const getChild = (key: string) => {
        if (!(key in children)) throw new Error(MISSING_KEY_ERROR(key));
      ...
      const deleteKey = keyToDelete => {
        ...
        for (const key in state)
          if (key != keyToDelete) out[key] = state[key];
